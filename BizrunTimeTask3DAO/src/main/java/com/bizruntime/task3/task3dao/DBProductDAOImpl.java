...
public class DBProductDAOImpl implements IProductDAO{
	...
	public List<Product> getAllProducts() throws DataReadException, DataResourcesNotFoundException {
		...
		String query = "SELECT product.productId,product.productName,product.productCategory,product.price,GROUP_CONCAT(availablecity.city) FROM product INNER JOIN availablecity WHERE product.productId = availablecity.productId GROUP BY availablecity.productId";
